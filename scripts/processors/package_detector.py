...
- Input: APK files analyzed via Androguard library
- Output: Tuple of (manifest_package, detected_package, confidence_stats)
- Used by: batch_generator for .methods file processing, reachability analysis
...
class PackageDetector:
    ...
    def detect_package(self, apk_path: str, verbose: bool = False) -> Tuple[str, str, Dict]:
        ...
        Args:
            ...
            verbose: Print debug information including component counts,
                    unique packages, and detection reasoning.

        Returns:
            Tuple of (manifest_package, detected_package, stats) where:
            - manifest_package: Package from AndroidManifest.xml
            - detected_package: Detected actual package (may equal manifest)
            - stats: Dictionary containing:
                - total_components: Count of all components found
                - app_components: Count of non-framework components
                - unique_packages: Count of unique packages detected
                - confidence: 'high', 'medium', or 'low'
                - reason: Detection strategy used (e.g., 'single_package')
                - all_packages: List of unique packages found
                - similarity_score: String similarity score if applicable
                - game_engine: Detected engine name or None
        ...
        Raises:
            Exception: If APK cannot be parsed by Androguard.
        """
        # Load APK
        apk = APK(apk_path)
...
def main() -> None:
    """
    Command-line interface for package detection.

    Usage: python package_detector.py <apk_path>

    Detects package name of provided APK and prints detection results
    with confidence levels and reasoning.
    """
    import sys
